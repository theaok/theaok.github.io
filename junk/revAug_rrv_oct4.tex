...
Thank you, please see below how we addressed each of your comments. 

\cc{My specific comments and suggestions follow:

1.      Sample selection and country information:
The paper aims to explore how Covid-19 changed subjective well-being (SWB) in cities compared to smaller places. For this inquiry the authors select a sample of countries that can be characterized as severely affected. To what extent the
countries are affected is measured by the number of infected and deaths due to Covid-19. In my opinion, the paper would benefit from clearly stating this earlier in the text, explaining the country selection process in detail, and
justifying the decisions made.}

The country selection process fits better in the data section; but early on, at the end of introduction we now added the following:\\
...
URY''--the information is substantial, but not perfect or complete and while much information is comparable, there is also a qualitative component not directly comparable. % we were unable to find more or
% better information\\ 
...
That's a great point. We now better orient the reader regarding sample selection early on in the manuscript by naming the countries in the abstract and clearly naming the three cases at the end of the introduction.\\
...
Still, you have a point that ``the reader will be interested
in seeing if these differences are statistically significant''
...
Still, the results are more than just associations---we control for predictors of DV; we conduct robustness checks
 with different specifications, and have before-after (pre and during pandemic)
...
Yes, totally! It is arguably also the reduction in urban amenities availability! We had hinted at it earlier, but now elaborate in the discussion section--thank you!
...
Fixed (in fact all tables 1-3 had the same heading), thank you for pointing that out. 
...
Yes, we fixed that: it is pre versus during pandemic; not post.\\

Reviewer 2 also says they are satisfied with the revision. 
...
and have an understanding of urbanization, happiness, and covid19 in the US.\\

Still, per your suggestion, we cut on the US where it was less relevant.\\

And we have rephrased the last paragraph so it is better connected to the article.\\
...
One would imagine, still the samples are quite large during the pandemic across urbanicity--see crosstabs in the appendix.\\
...
Morocco and Venezuela are very special cases as explained in the data section, that's why they are postponed to the appendix, these are auxiliary findings only.
...
Done; Please note that the bib style used displays only volume without number so its 56; not 56(12); if the journal requires number as well, it will be added in the publishing/proofreading stage.  

Thank you again for your comments and helping make our paper much stronger! 
...
% this aint working:(
\includepdf[pages={-}]{old-new3.pdf}%don't forget to latex diff.tex!
