...
%RV COMMENT: I think that the issue with the title is the use of misanthropy, the reviewer hints that what we are measuring is distrust and not misanthropy, which is why s/he says the title is misleading. I invented a new word =) misanthropolis 
%alternatively we could do: 
% Misanthropolis: Do Cities Promote Misanthropy?
% or something else if you dont like these suggestions
%%LOL me gusta misanthropolis :)))
}
\author{
% ANONYMOUS
...
%%%i guess move back to appendix! and see initial language that it may but doesnt have to be!!
...
%Some studies, however, did not find negative effects of density or crowding and results were%  mixed \citep{collette1976urban}. While it seems to be reasonable to assume
%  that density and crowding are positivelty related, some studies do not find %  this to be the case\citep{webb1975meaning,rodgers1982density}.
%ADAM: These are all very old studies, do we have current ones that confirm these claims? if not, I would leave it out, I don't think this adds to the paper, and the reviewer was already critical that we have "dated" evidence. 
%%%%%%%%%%%%%%%%%UPTO HERE%%%%%%%%
...
Hence, the only quantitative study solely focusing on the urbanicity-misanthropy relationship is \citet{wilson85}.%\footnote{This study is unavailable online, and we were only able to read it by requesting it through an interlibrary loan.}
%not a good idea to say that it isn't well cited, might indicate that there's no interest in this area and that ours will not be cited as well.
%
...
%I checked, smith controlled for health, but NOT for Political ideology or SWB. Couldn't find Wilson's study. 
...
%\footnote{Some prourbanists make the case that
...
There's reason to believe that future research in other developed countries will find similar results, especially in Western countries where people are unhappier in the largest metropolitan areas, and therefore more likely to be misanthropic. %\citep{%TO DO}.
%TODO CITATION
...
%\newpage
%\theendnotes
%\bibliography{C:\Users\rubia\Downloads\old-new.bbl}

%TODO please add to Bibliography: 
%@article{ala18,
%  title={Spatial nature of urban well-being},
%  author={Ala-Mantila, Sanna and Heinonen, Jukka and Junnila, Seppo and Saarsalmi, Perttu},
